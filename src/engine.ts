...
export class PlayerEngine {
  ...
  private async fillInitialStreamingBuffer(signal:AbortSignal,loadGeneration:number,seekGeneration:number):Promise<void>{
    ...
    const current=()=>!signal.aborted&&!this.destroyed&&loadGeneration===this.loadGeneration&&seekGeneration===this.seekGeneration;
    const fps=this.essenceIndex.frameRate,targetFrame=Math.min(Math.ceil(this.durationValue*fps)-1,Math.ceil(this.videoAheadSeconds*fps)-1);
    let videoStart=0,audioStart=0;
    while(current()&&(this.queuedThroughFrame<targetFrame||this.streamingAudioSupported&&!this.audioExhausted&&this.audioQueuedThroughTime<this.videoAheadSeconds)){
      const previousVideo=this.queuedThroughFrame,previousAudio=this.audioQueuedThroughTime;
      const jobs:Promise<boolean>[]=[];
      if(this.queuedThroughFrame<targetFrame){videoStart=Math.max(videoStart,this.queuedThroughFrame+1);jobs.push(this.fillStreaming(videoStart,signal,loadGeneration,seekGeneration));}
      if(this.streamingAudioSupported&&!this.audioExhausted&&this.audioQueuedThroughTime<this.videoAheadSeconds){audioStart=Math.max(audioStart,this.audioQueuedThroughTime);jobs.push(this.fillStreamingAudio(audioStart,signal,loadGeneration,seekGeneration));}
