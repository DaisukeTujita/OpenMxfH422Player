...
export function selectTimecodeTrack(timecodes: MxfTimecodeInfo[], logger: TimecodeLogger = console): MxfTimecodeInfo | undefined {
  ...
  const resolved = usable.filter(value => value.packageReferenceResolved);
  const selected = resolved.find(value => value.packageKind === "material") ?? resolved.find(value => value.packageKind === "source") ?? usable[0];
  if (timecodes.length > 1) logger.warn(`[H422Player] multiple Timecode Tracks detected (${timecodes.length}); ${resolved.length ? "resolved package priority" : "package unresolved; KLV-order fallback"}`);
...
export class PlayerEngine {
  ...
  async load(source: File|Blob|string): Promise<void> {
    ...
    let localReader:(RandomAccessReader & {destroy():void})|undefined;try {
      ...
      this.timecodeSelectionReason=timecodeInfo?.packageReferenceResolved?`${timecodeInfo.packageKind === "material" ? "Material Package" : "Source Package"}の参照を解決して選択`:timecodeInfo?"Package参照を解決できないためKLV検出順で選択":"Timecode Trackなし"; metadata.mediaInfo.timecodeSelectionReason=this.timecodeSelectionReason;
