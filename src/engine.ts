...
export class PlayerEngine {
  ...
  private yuvFramePool=new Map<string,Yuv422Frame[]>(); private rgbaFramePool=new Map<string,Uint8ClampedArray[]>();
  ...
  getDiagnostics():PlayerDiagnostics { const stats=(this.reader as any)?.getStats?.()??{};const active=this.audio&&this.scheduledAudio.find(range=>range.contextStartTime<=this.audio!.currentTime&&range.contextStartTime+range.mediaEndTime-range.mediaStartTime>=this.audio!.currentTime);const audioTime=active&&this.audio?active.mediaStartTime+this.audio.currentTime-active.contextStartTime:null;const pools=[...(this.yuvFramePool?.values()??[]),...(this.rgbaFramePool?.values()??[])];return {mode:this.mode,videoRenderMode:this.videoRenderMode,fileSize:this.fileSize,bytesLoaded:Number(stats.bytesLoaded??0),underlyingReadCount:stats.underlyingReadCount??0,cacheBytes:stats.cachedBytes??0,videoQueueFrames:this.frames.length,videoQueueStart:this.frames[0]?.time??null,videoQueueEnd:this.frames.at(-1)?.time??null,scheduledAudioRanges:this.mode==="streaming"?this.scheduledAudio.length:(this.audioSource?1:0),loadGeneration:this.loadGeneration,seekGeneration:this.seekGeneration,streamingAudioSupported:this.streamingAudioSupported,selectedAudioTrackNumber:this.selectedAudioTrackNumber??null,audioSampleRate:this.audioSampleRate??null,audioChannels:this.audioChannels??null,audioQueueStart:this.audioChunks?.[0]?.mediaStartTime??null,audioQueueEnd:this.audioChunks?.at(-1)?.mediaEndTime??null,audioVideoDriftMs:audioTime===null?null:(audioTime-this.currentTime)*1000,audioBytesLoaded:this.audioBytesLoaded,audioQueuedThroughTime:this.audioQueuedThroughTime,audioExhausted:this.audioExhausted,lastPlayableAudioTime:this.streamingAudioSupported?this.lastAudioTime:null,audioFormatBasis:this.audioFormatBasis,requestedTimecode:this.requestedTimecode,requestedFrame:this.requestedFrame,actualDisplayedFrame:this.actualDisplayedFrame,seekStartFrame:this.seekStartFrame,prerollFrames:this.prerollFrames,seekSource:this.seekSource,seekReadBytes:this.seekReadBytes,seekElapsedMs:this.seekElapsedMs,selectedTimecodeTrack:this.timecodeInfo?"unresolved":null,timecodeSelectionReason:this.timecodeSelectionReason,videoDecodedFrames:this.videoDecodedFrames,videoDecodeMs:this.videoDecodeMs,videoColorConvertMs:this.videoColorConvertMs,videoUploadMs:this.videoUploadMs,decoderExecution:"libav-worker",adaptiveVideoAheadSeconds:this.adaptiveVideoAheadSeconds??this.videoAheadSeconds,adaptiveRefillThresholdSeconds:this.adaptiveRefillThresholdSeconds??this.refillThresholdSeconds,lastChunkDecodeMs:this.lastChunkDecodeMs??0,pooledVideoFrames:pools.reduce((total,pool)=>total+pool.length,0)}; }
  private publishDiagnostics(){this.callbacks.diagnostics?.(this.getDiagnostics());}
  private frameKey(width:number,height:number){return `${width}x${height}`;}
  private acquireYuvFrame(width:number,height:number):Yuv422Frame { const key=this.frameKey(width,height),pool=this.yuvFramePool?.get(key),reused=pool?.pop();return reused??{width,height,y:new Uint8Array(width*height),u:new Uint8Array(Math.ceil(width/2)*height),v:new Uint8Array(Math.ceil(width/2)*height)}; }
  private acquireRgba(width:number,height:number):Uint8ClampedArray { const key=this.frameKey(width,height),pool=this.rgbaFramePool?.get(key);return pool?.pop()??new Uint8ClampedArray(width*height*4); }
  private releaseFrameStorage(frame:ImageData|Yuv422Frame):void { const key=this.frameKey(frame.width,frame.height);if("u" in frame&&"v" in frame){const pools=this.yuvFramePool??=new Map(),pool=pools.get(key)??[];if(pool.length<90){pool.push(frame);pools.set(key,pool);}}else{const pools=this.rgbaFramePool??=new Map(),pool=pools.get(key)??[];if(pool.length<30){pool.push(frame.data);pools.set(key,pool);}} }
  private clearFrames():void { for(const item of this.frames)this.releaseFrameStorage(item.frame);this.frames=[]; }
  private evictFramesBefore(time:number):void { const retained:RenderFrame[]=[];for(const item of this.frames){if(item.time>=time)retained.push(item);else this.releaseFrameStorage(item.frame);}this.frames=retained; }
  private adaptStreamingBuffer(decodeMs:number,inputFrames:number,frameRate:number):void { this.lastChunkDecodeMs=decodeMs;if(this.videoRenderMode!=="yuv-webgl"||inputFrames<1)return;const decodeSeconds=decodeMs/1000,mediaSeconds=inputFrames/frameRate,maxAhead=Math.max(this.videoAheadSeconds,this.chunkSeconds*3);if(mediaSeconds<=0)return;const desired=this.chunkSeconds+decodeSeconds*2+.5;this.adaptiveVideoAheadSeconds=Math.min(maxAhead,Math.max(this.videoAheadSeconds,Math.ceil(desired/this.chunkSeconds)*this.chunkSeconds));this.adaptiveRefillThresholdSeconds=Math.min(this.adaptiveVideoAheadSeconds-.25,Math.max(this.refillThresholdSeconds,decodeSeconds*1.75+.75)); }
  ...
  private async fillStreaming(targetFrame:number,signal:AbortSignal,loadGeneration:number,seekGeneration=this.seekGeneration,decodeStartFrame?:number):Promise<boolean>{
    ...
    const canContinue=decodeStartFrame===undefined&&target===this.queuedThroughFrame+1&&this.hasStreamingVideoDecoder(loadGeneration,seekGeneration);
  ...
  private hasStreamingVideoDecoder(loadGeneration:number,seekGeneration:number):boolean { const state=this.streamingVideoDecoder;return Boolean(state&&!state.disposed&&!state.disposeRequested&&state.loadGeneration===loadGeneration&&state.seekGeneration===seekGeneration&&state.av===this.libav&&state.codecId===this.videoCodecId); }
  private async freeStreamingVideoDecoder(state:StreamingVideoDecoder):Promise<void>{if(state.disposed)return;state.disposed=true;await state.av.ff_free_decoder(state.ctx,state.pkt,state.frame);}
  private invalidateStreamingVideoDecoder():void{const state=this.streamingVideoDecoder;this.streamingVideoDecoder=undefined;if(!state)return;state.disposeRequested=true;if(!state.busy)void this.freeStreamingVideoDecoder(state).catch(error=>console.warn("[H422Player] streaming decoder cleanup failed",error));}
  private async decodeStreamingVideo(chunks:Uint8Array[],mediaFrames:number[],frameRate:number,flush:boolean,loadGeneration:number,seekGeneration:number):Promise<RenderFrame[]> {
    ...
    try {
      ...
      for(let i=0;i<decoded.length;i++){const f=decoded[i];if(!f.data)continue;if(f.format!==av.AV_PIX_FMT_YUV422P||!f.layout||f.layout.length<3)throw new Error(`Expected yuv422p planes from MPEG-2 decoder (pixel format ${f.format??"unknown"})`);const reusable=this.acquireYuvFrame(f.width,f.height),plane=(index:number,width:number,output:Uint8Array)=>{const layout=f.layout![index];for(let row=0;row<f.height;row++)output.set(f.data!.subarray(layout.offset+row*layout.stride,layout.offset+row*layout.stride+width),row*width);};const chromaWidth=Math.ceil(f.width/2);plane(0,f.width,reusable.y);plane(1,chromaWidth,reusable.u);plane(2,chromaWidth,reusable.v);let renderFrame:ImageData|Yuv422Frame;if(this.videoRenderMode==="yuv-webgl")renderFrame=reusable;else{const convertStarted=performance.now(),rgba=yuv422pToRgba(reusable.y,reusable.u,reusable.v,f.width,f.height,this.acquireRgba(f.width,f.height));chunkConvertMs+=performance.now()-convertStarted;renderFrame=new ImageData(rgba as unknown as ImageDataArray,f.width,f.height);this.releaseFrameStorage(reusable);}const decodedPts=f.pts===undefined?Number.NaN:Number(f.pts);const mediaFrame=Number.isSafeInteger(decodedPts)&&decodedPts>=0&&decodedPts<maxMediaFrame?decodedPts:mediaFrames[i]??i;frames.push({frame:renderFrame,time:mediaFrame/frameRate,mediaFrame});}
      this.adaptStreamingBuffer(chunkDecodeMs,packets.length,frameRate);this.videoDecodedFrames+=frames.length;this.videoColorConvertMs+=chunkConvertMs;console.info("[H422Player] video performance",{renderMode:this.videoRenderMode,decoderExecution:"libav-worker",inputPackets:packets.length,decodedFrames:frames.length,decodeMs:Number(chunkDecodeMs.toFixed(1)),colorConvertMs:Number(chunkConvertMs.toFixed(1)),adaptiveAheadSeconds:this.adaptiveVideoAheadSeconds??this.videoAheadSeconds??0,adaptiveRefillSeconds:Number((this.adaptiveRefillThresholdSeconds??this.refillThresholdSeconds??0).toFixed(2)),totalDecodeMs:Number((this.videoDecodeMs??0).toFixed(1)),totalColorConvertMs:Number((this.videoColorConvertMs??0).toFixed(1))});
    ...
    state.busy=false;
    if(state.disposeRequested)try{await this.freeStreamingVideoDecoder(state);}catch(error){if(failure===undefined)failure=error;}
  ...
  private drawAt(t:number,exact=false):RenderFrame|undefined{const requested=Math.round(t*(this.essenceIndex?.frameRate??XDCAM_FRAME_RATE));const f=this.mode==="streaming"?(exact?this.frames.find(item=>item.mediaFrame===requested):[...this.frames].reverse().find(item=>item.time<=t+.001)):this.frames[Math.min(this.frames.length-1,Math.floor(t*XDCAM_FRAME_RATE))];if(f)this.drawFrame(f.frame);return f;}
