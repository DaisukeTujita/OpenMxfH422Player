...
export class PlayerEngine {
  ...
  private requestFill(t:number,force=false){if(this.mode!=="streaming"||!this.essenceIndex||this.filling)return;const fps=this.essenceIndex.frameRate,ahead=(this.frames.at(-1)?.time??t)-t;if(!force&&ahead>=this.refillThresholdSeconds)return;const controller=new AbortController();this.fillController=controller;const loadGeneration=this.loadGeneration,seekGeneration=this.seekGeneration,start=Math.max(Math.floor(t*fps),this.queuedThroughFrame+1);const promise=this.fillStreaming(start,controller.signal,loadGeneration,seekGeneration).then(applied=>{if(!applied||controller.signal.aborted||this.filling!==promise)return;const remaining=(this.frames.at(-1)?.time??t)-t;if(remaining<this.videoAheadSeconds&&this.queuedThroughFrame<Math.ceil(this.durationValue*fps)-1){this.filling=undefined;this.fillController=undefined;this.requestFill(t,true);return;}if(this.buffering){this.drawAt(this.pausedAt);const resume=this.resumeAfterBuffer;this.setBuffering(false);if(resume){this.startedAt=performance.now()-this.pausedAt*1000;this.setStatus("playing");this.tick();}}}).catch(e=>{if((e as Error).name!=="AbortError"&&this.filling===promise)this.callbacks.error(e as Error);}).finally(()=>{if(this.filling===promise){this.filling=undefined;if(this.fillController===controller)this.fillController=undefined;}});this.filling=promise;}
  ...
  private tick():void{if(this.destroyed||this.status!=="playing")return;const t=this.currentTime;if(this.mode==="streaming"){this.frames=this.frames.filter(f=>f.time>=t-this.retainBehindSeconds);const hasFuture=this.frames.some(frame=>frame.time>=t);if(!hasFuture&&t<this.durationValue){this.pausedAt=Math.min(t,this.durationValue);this.resumeAfterBuffer=true;cancelAnimationFrame(this.raf);this.raf=0;this.setStatus("buffering");this.setBuffering(true);this.requestFill(this.pausedAt,true);return;}this.requestFill(t);}this.drawAt(t);this.emitTime(t);if(t>=this.durationValue){this.pausedAt=this.durationValue;this.stopAudioSource();void this.audio?.suspend();cancelAnimationFrame(this.raf);this.raf=0;this.setStatus("ended");return;}this.raf=requestAnimationFrame(()=>this.tick());}
