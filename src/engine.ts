...
async function loadCustomLibAV(base: string): Promise<LibAV> {
  const url = `${base.replace(/\/$/, "")}/libav-h422.mjs`;
  // The generated custom frontend is a deployment asset rather than a bundled dependency.
  const module = await import(/* @vite-ignore */ url) as { default?: { LibAV(options: object): Promise<unknown> }; LibAV?: (options: object) => Promise<unknown> };
  const factory = module.default?.LibAV ?? module.LibAV;
  if (!factory) throw new Error(`Invalid custom libav.js frontend: ${url}`);
  return await factory({ base: base.replace(/\/$/, ""), noworker: false }) as LibAV;
}
