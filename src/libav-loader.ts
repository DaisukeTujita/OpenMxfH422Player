...
export async function loadCustomLibAV(base: string): Promise<LibAV> {
  ...
  try {
    ...
    // webpackIgnore covers webpack/Turbopack (Next.js); @vite-ignore covers Vite. Neither bundler
    // honors the other's magic comment, so both must be present for this dynamic blob: import to
    // reach the native import() at runtime instead of being rewritten by the bundler.
    const module = await import(/* webpackIgnore: true */ /* @vite-ignore */ moduleUrl) as { default?: { LibAV(options: object): Promise<unknown> }; LibAV?: (options: object) => Promise<unknown> };
